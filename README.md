...
* **Strict Security Compliance (SEI CERT C):** The codebase formally adheres to the SEI CERT C secure coding standards (`cert-*`). The architecture mandates zero dynamic memory (`malloc`/`free`) on the hot path, strictly bounds-checked string formatting, and fully audited pointer arithmetic.
